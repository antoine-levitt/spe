...
for line_ndx = 1:Nlns          % This section sets the adjacency matrix entry A(Lnode,Rnode) = edge_id.
    my_line = dot_lines(line_ndx,:);
    Ddash_pos = strfind(my_line, ' -- ') + 1;  % double dash positions
    arrow_pos = strfind(my_line, ' -> ') + 1;  % arrow  dash positions
    tokens = strread(my_line,'%s','delimiter',' "');
    left_bound = 1;
    for dash_pos = [Ddash_pos arrow_pos];  % if empty - not a POS line
        Lnode = sscanf(my_line(left_bound:dash_pos -2), '%s');
        Rnode = sscanf(my_line(dash_pos +3 : length(my_line)-1),'%s',1);
...
for line_ndx = unread        % Look for node's coordiantes among the 'unread' lines.
    my_line = dot_lines(line_ndx,:);
    bra_pos  = strfind(my_line, '[');      % has to have "[" if it has the lable
    pos_pos = strfind(my_line, 'pos');     % position of the "pos"
    for node = 1:Nvrt     % look through the list of labels 
        %  THE NEXT STATEMENT we assume no label is substring of any other label
        lbl_pos = strfind(my_line, labels{node});
    ...
    if (~isempty(pos_pos) & lst_node)   % this line contains SOME position  
        [node_pos] = sscanf(my_line(pos_pos:length(my_line)), ' pos  = "%g,%g"')';
