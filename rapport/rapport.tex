...
En pratique, on a $\alpha = 0,1$ pour le graphe Zachary et $\alpha = 0,4$ pour les
...
Nous commençons par analyser les résultats qualitatifs
...
Nous voyons ici apparaître 2 phases (\fig{fig:evol}):
\begin{itemize}
...
\paragraph{Influence du forçage}
...
\begin{figure}[htb]
	\begin{center}
		...
		\caption{Évolution de l'écart type des opinions pour différentes valeurs de $\rho$}
		\label{num_coupure}
	\end{center}
\end{figure}

Dans les 2 premiers cas, $\rho$ est assez éloigné de $\lambda_2$, on a bien coupure si $\rho < \lambda_2$.
Dans le troisième cas, on a pris $\rho \leq \lambda_2$, il y a théoriquement coupure, mais seulement après un grand nombre
d'itérations. L'écart type est alors de l'ordre de la précision machine et on a des erreurs lors de la construction du graphe
de voisinage $N'$.
