...
Nous cherchons des communautés apparaissant dans la réalité, dans ce but
nous utilisons des graphes réels. Des graphes aléatoires ne présentent pas
a priori une telle structure.

Nous utilisons principalement deux graphes, représentés \fig{exemples_graphes}. Le premier
est le réseau d'amitié dans un club de karaté d'une université américaine dans les années
70, contenant 34 membres \cite{zachary}. Le second est le graphe d'interactions d'une
communauté de 62 dauphins \cite{dolphins}. Il a été observé une séparation de ce groupe
après le départ d'un individu clé.
...
$R$ est choisi relativement grand devant la taille de la distribution initiale
($R\approx10$) pour éviter l'influence lors de la phase transitoire. Enfin, $\alpha$
est choisi de manière à avoir une simulation stable (voir section \ref{mod_discret}).
En pratique, on a $\alpha = 0,08$ pour le graphe Zachary et $\alpha = 0,4$ pour les
dauphins. On effectue $\approx 400$ itérations, les résultats sont alors peu variants,
et la simulation rapide.

\subsection{Détection des groupes} Sans s'attacher à une définition formelle de la notion
de communauté (qui peut être complexe) on peut définir un groupe. À l'instant $t$, on
considère graphe partiel $N'(t) = \bigcup_i N'_i(t)$. Ce graphes présente une ou plusieurs
composantes connexes, qui représentes les ensembles d'agents communiquant encore entre eux
(directement ou pas) à l'instant $t$. Ces

\subsection{Résultats numériques} Nous commençons par analyser les résultats qualitatifs
pour le modèle, avec $\rho$ de l'ordre de $\lambda_2$. On observe des groupes au sein
desquels l'opinion est globalement homogène. Cette structure de groupe semble être peu
sensible à la répartition initiale des opinions puisqu'elle apparaît très souvent sur
plusieurs simulations (voir \fig{res_3cs}).

\begin{figure}[htb]
	\begin{center}
		\includegraphics[width=.4\textwidth]{za-m1-3}
		\includegraphics[width=.4\textwidth]{do-m1-3}
		\caption{Exemple de groupes apparaissant lors de la simulation}
		\label{res_3cs}
	\end{center}
\end{figure}
...
\subparagraph{Influence du forçage}

Maintenant nous nous intéressons à la bifurcation introduite en section \ref{forcage}.
...
Nous utiliseons le graphe de Zachary, pour lequel $\lambda_2 \approx 0,9471$.
Le nombre de groupe est calculé en faisant la moyenne sur 10 simulations.

\begin{figure}[htb]
	\begin{center}
		%\includegraphics[width=\textwidth]{do-m1-3}
		\caption{Bifurcation pour $\rho < \lambda_2$}
		\label{bifu_map}
	\end{center}
\end{figure}

Il n'y a qu'un groupe pour $\rho \geq \lambda_2$. Pour $\rho < \lambda_2$, on l'apparition
de quelques groupes, jusqu'à $0,94$ où on a brusquement plus de 20 groupes. La
bifurcation a bien lieu, mais pour $\rho < \lambda_2$. Pour $\rho \precsim \lambda_2$, les
vitesses de décroissances de $R\rho^t$ d'une part et de la différence entre les opinions
d'une autre par sont très proches. Il faut donc un grand nombre d'itérations pour arriver
à la coupure. On a alors des erreurs numériques. En effet, lorsque la différence de
valeur entre les différents n\oe uds atteint la précision machine, des erreurs de
comparaison apparaissent lors du calcul de N', des coupures n'ont pas lieu alors qu'elles
le devraient.

\begin{figure}[htb]
	\begin{center}
		\includegraphics[width=.4\textwidth]{var_rho_ok_2-17.eps}
		\includegraphics[width=.4\textwidth]{var_rho_nok_2-23.eps}
		\caption{Évolution de la variance et coupure}
		\label{num_coupure}
	\end{center}
\end{figure}


Dans la section suivante, en plus des méthodes << ad hoc >> pour la recherche de communautés,
nous introduisons un nouveau modèle pour homogénéiser la taille des groupes.
...
faut tout d'abord définir rigoureusement ce qu'on entend par
``naturelle''. Un critère qui peut être intéressant est le nombre
d'arêtes entre les deux groupes : on cherche la partition de $E$ en
deux groupes $E_1$ et $E_2$ qui minimise ce nombre d'arêtes $n_a$.
...
Modèle normalisé, fonction objectif, partitionnement spectral, (modularité ?)
